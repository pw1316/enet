...
{
    if (packet->flags & ENET_PACKET_FLAG_KCP && peer->state == ENET_PEER_STATE_CONNECTED)
    {
    ikcp_send(peer->kcpcb, packet->data, packet->dataLength);
    ikcp_flush(peer->kcpcb);
    return 0;
    }
...
enet_peer_on_connect(ENetPeer * peer)
{
    if (peer->state != ENET_PEER_STATE_CONNECTED && peer->state != ENET_PEER_STATE_DISCONNECT_LATER)
    {
        ...
        //create kcp when connnetion setup
        if (peer->kcpcb == NULL)
        {
            ENetUser* user = (ENetUser*)enet_malloc(sizeof(ENetUser));
            if (user == NULL)
                return;
            user->socket = peer->host->socket;
            user->addr = peer->address;
            user->peerID = peer->outgoingPeerID;
            peer->kcpcb = ikcp_create(peer->connectID, (void*)user);
            if (peer->kcpcb == NULL)
                return;
            ikcp_wndsize(peer->kcpcb, 128, 128);
            ikcp_nodelay(peer->kcpcb, 1, 10, 2, 1);
            peer->kcpcb->rx_minrto = 5;
            peer->kcpcb->output = kcp_udp_output;
...
enet_peer_on_disconnect(ENetPeer * peer)
{
    if (peer->state == ENET_PEER_STATE_CONNECTED || peer->state == ENET_PEER_STATE_DISCONNECT_LATER)
    {
        ...
        //release kcp when disconnect
        if (peer->kcpcb != NULL)
        {
            ikcp_release(peer->kcpcb);
            peer->kcpcb = NULL;
        }
        peer->kcpNeedDispatch = 0;
    }
