...
//kcp output definition
int kcp_udp_output(const char *buf, int len, ikcpcb *kcp, void *user)
{
    enet_uint8 headerData[sizeof(ENetProtocolHeader)];
    ENetProtocolHeader* header = (ENetProtocolHeader*)headerData;
    ENetUser* enetUser = (ENetUser*)user;
    ENetBuffer buffer[2];

    header->peerID = ENET_HOST_TO_NET_16(enetUser->peerID);

    buffer->data = headerData;
    buffer->dataLength = (size_t)& ((ENetProtocolHeader *)0)->sentTime;

    buffer[1].data = (char*)buf;
    buffer[1].dataLength = len;

    enet_socket_send(enetUser->socket, &enetUser->addr, buffer, 2);
    return 0;
}
...
{
    while (!enet_list_empty(&host->kcpDispatchQueue))
    {
        ENetPeer* peer =(ENetPeer*) ((char*)(ENetPeer*)enet_list_remove(enet_list_begin(&host->kcpDispatchQueue)) - (size_t)&((ENetPeer*)0)->kcpDispatchList);
        int hr;
        ENetPacket* packet;
        enet_uint8* data;

        peer->kcpNeedDispatch = 0;

        switch (peer->state)
        {
        case ENET_PEER_STATE_CONNECTED:
            hr = ikcp_recv(peer->kcpcb, host->kcpBuffer, ENET_KCP_BUFF_SIZE);
            if (hr > 0)
            {
                packet = enet_malloc(sizeof(ENetPacket));
                if (packet == NULL)
                    return -1;
                data = (enet_uint8*)enet_malloc(hr * sizeof(enet_uint8));
                if (data == NULL)
                    return -1;
                memcpy(data, host->kcpBuffer, hr);
                packet->data = data;
                packet->dataLength = hr;
                packet->flags = ENET_PACKET_FLAG_KCP;
                ...
                if (ikcp_peeksize(peer->kcpcb) > 0)
                {
                    peer->kcpNeedDispatch = 1;
                    enet_list_insert(enet_list_end(&host->kcpDispatchQueue), &peer->kcpDispatchList);
                }
                return 1;
            }
            else
            {
                printf(">>>kcp recv error<<<\n");
            }
...
static void
enet_kcp_update(ENetHost *host)
{
    ENetPeer* currentPeer;
    for (currentPeer = host->peers; currentPeer < &host->peers[host->peerCount]; ++currentPeer)
    {
        if (currentPeer->kcpcb != NULL)
        {
            ikcp_update(currentPeer->kcpcb, enet_time_get());
        }
    }
}
...
enet_host_service(ENetHost * host, ENetEvent * event, enet_uint32 timeout)
{
    ...
    do
    {
        //update all connneted peer's kcp
        enet_kcp_update(host);
