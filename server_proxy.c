...
void send_data(intptr_t ppeer, const char *data, uint32_t length)
{
  ENetPeer *peer = (ENetPeer*)(ppeer);
  if (peer != NULL)
  {
    ENetPacket *pkt = enet_packet_create(data, length, ENET_PACKET_FLAG_RELIABLE);
...
int tick(intptr_t *peer, intptr_t *buf, uint32_t *length)
{
  ...
  ENetEvent event;
  while (enet_host_service(g_enet_host, &event, 0) > 0)
  {
    switch (event.type)
    {
